...
categories<-c("Education","Employment","Volunteering","Projects")
...
for (c in categories){
  ...
  for (i in 1:nrow(cv_sub)){
    entry<-cv_sub[i,]
      ...
      #Setting up Visuals (if present)  
      size.mod<-"{width=75%}"
      
      V1<-paste(
        ifelse(is.na(entry$V1)==T,"",
               paste("![]","(visuals/",entry$V1,entry$V1_Type,")",
                     size.mod,"\n","\n",entry$V1_Desc,sep="")),sep="")
      V2<-paste(
        ifelse(is.na(entry$V2)==T,"",
               paste("![]","(visuals/",entry$V2,entry$V2_Type,")",
                     size.mod,"\n","\n",entry$V2_Desc,sep="")),sep="")
      V3<-paste(
        ifelse(is.na(entry$V3)==T,"",
               paste("![]","(visuals/",entry$V3,entry$V3_Type,")",
                     size.mod,"\n","\n",entry$V3_Desc,sep="")),sep="")
    ...
    #Printing Entry for Markdown
      ...
      if(c=="Projects"){
        ...
        cat("##",entry$Organization,"{.tabset}", "\n","\n")+
          cat(timing," (",period,") ",location," ",org.link,"\n","\n",sep="")}
  
      cat("###", "Description", "\n","\n")
        cat(description, "\n","\n")
        
      #Checking for visuals
      if(V1==""){""} else {
        cat("###", "Visuals", "\n", "\n")
          cat(V1,V2,V3,"\n","\n")
      }
      ...
      #Empty Active Tab with Title to hide all   
      cat("###", "-","{.active}", "\n","\n")
        cat("**",entry$Title,"**", "\n","\n",sep="")
